...
export default function PolicyNavigator() {
  const { policyDocuments } = useCampusOps();
                  ...
                  <div className="space-y-1">
                    {TABS.map(({ id, label, Icon }) => (
                      <button key={id} onClick={() => setMode(id)}
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors text-left ${mode === id ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-secondary"
                          }`}>
                  ...
                  <motion.div key={mode} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {mode === "eligibility" && (
                      <div className="flex-1 overflow-y-auto p-5 space-y-5">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {ELIGIBILITY_OPTIONS.map((opt) => (
                              <button key={opt} onClick={() => setCheckFor(opt)}
                                className={`px-3 py-1.5 rounded-xl text-sm border font-medium transition-colors ${checkFor === opt
                        ...
                        {eligibilityResult && (
                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                            <div className={`rounded-xl p-5 border ${eligibilityResult.eligible
                                ? "bg-emerald-500/5 border-emerald-500/30"
                                : "bg-destructive/5 border-destructive/30"
                              }`}>
                              <div className="flex items-center gap-3 mb-4">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${eligibilityResult.eligible ? "bg-emerald-500/10" : "bg-destructive/10"
                                  }`}>

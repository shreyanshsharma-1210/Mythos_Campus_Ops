...
import { HunterProfile, Quest, HunterStats, Buff } from './types';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (currentPage) {
      case 'map':
        return <div className="w-full h-[80vh]"><WorldMap /></div>;
      case 'chronicle':
        return <div className="w-full h-[80vh] max-w-3xl mx-auto"><ChronicleFeed /></div>;
      case 'vault':
        return <div className="w-full h-[80vh] max-w-4xl mx-auto"><InventoryVault /></div>;
      case 'home':
      default:
        return (
  ...
  return (
    <div className="relative min-h-screen w-full font-sans text-slate-800 overflow-x-hidden flex flex-col">
      <Background />

      <header className="absolute top-0 w-full z-50 px-6 py-4 flex justify-between items-center text-slate-400 pointer-events-none">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 pointer-events-auto">
          <h1 className="text-2xl md:text-3xl font-display font-black tracking-tighter text-slate-800 uppercase flex items-center gap-2">
      ...
      <main className="flex-1 w-full max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-12 relative z-10 pt-20 pb-32">
        {renderContent()}
      </main>

      <footer className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-2xl p-6 z-50 pointer-events-none flex justify-center">
